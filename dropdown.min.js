...
function doSomething(element,settings){var parent=element.parentNode,dropdown=buildHTML(element,settings);parent.insertBefore(dropdown,element);element.style.height='0';element.style.visibility='hidden';element.style.margin='0';element.style.border='none';element.style.opacity=0.7;return dropdown;}
...
this.objects.push(object);}};Dropdown.prototype.apply=function(){var length=this.objects.length,i=0,j=0,elements=null;console.log('all Objects',this.objects);for(i=0;i<length;i=i+1){elements=document.querySelectorAll(this.objects[i].selector);for(j=0;j<elements.length;j=j+1){if(elements[j].tagName==='SELECT'){console.log('doSomething',this.objects[i]);doSomething(elements[j],this.objects[i]);}}}};Dropdown.prototype.version=function(){return VERSION;};Dropdown.prototype.options=function(){return defaults;};global.Dropdown=Dropdown;}(this,this.document));