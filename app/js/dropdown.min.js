...
view.className='dropdown-view';view.href='#';view.onclick=function(event){event.preventDefault();};view.setAttribute('tab-index','-1');option=document.createElement('span');if(Object.prototype.hasOwnProperty.call(settings,'placeholder')){element.selectedIndex=-1;option.className='placeholder';option.innerHTML=settings.placeholder;}else if(element.hasAttribute('data-placeholder')){element.selectedIndex=-1;option.className='placeholder';option.innerHTML=element.getAttribute('data-placeholder');}else{option.innerHTML=element[element.selectedIndex].innerHTML;option.setAttribute('data-selected',element.selectedIndex);}
view.appendChild(option);options.className='dropdown-options';options.onclick=dropdownClick;for(i=0;i<element.children.length;i=i+1){if(element.children[i].tagName==='OPTION'){option=document.createElement('li');option.className='option';option.setAttribute('data-index',index);if(element.selectedIndex===i){option.className+=' selected';}
if(element.children[i].hasAttribute('disabled')){option.className+=' disabled';}
...
if(optgroup.children[j].hasAttribute('disabled')){option.className+=' disabled';}
option.innerHTML=optgroup.children[j].innerHTML;option.onclick=optionClick;options.appendChild(option);index=index+1;}}}
